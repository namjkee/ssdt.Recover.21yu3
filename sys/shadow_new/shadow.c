...
#define   DEFAULT_LOG_FILE_NAME L"\\??\\C:\\test.log"
...
ExFreePool(w32kCopy );
	...
	ntStatus = RtlAppendUnicodeToString(&fileName,(PWSTR)DEFAULT_LOG_FILE_NAME);
    
	InitializeObjectAttributes(&objectAttributes,(PUNICODE_STRING)&fileName,OBJ_CASE_INSENSITIVE,NULL,NULL);
	
	ntStatus = ZwOpenFile(&FileHandle,GENERIC_READ,
						&objectAttributes,&ioStatus,
						FILE_SHARE_READ,FILE_RANDOM_ACCESS);

//  //DbgPrint("driver Opened..");
	if(NT_SUCCESS(ntStatus))
	{
//	//DbgPrint("File Opened..%X",FileHandle);
    ntStatus = ZwQueryInformationFile(FileHandle,&ioStatus, 
				    &StandardInfo, 
				    sizeof(FILE_STANDARD_INFORMATION), 
				    FileStandardInformation); 
	if (NT_SUCCESS(ntStatus)) {
//		//DbgPrint("Get fileinfo..%X",StandardInfo.EndOfFile.QuadPart);
	}
    byteOffset.QuadPart=0;
    while(byteOffset.QuadPart<StandardInfo.EndOfFile.QuadPart)
	{
//		//DbgPrint("Enter Reading..%X",FileHandle);
        ntStatus = ZwReadFile(FileHandle, NULL, NULL, NULL, &ioStatus,
                        	&ssdtaddr, sizeof(SST_ENTRY), &byteOffset, NULL);
		
// 		//DbgPrint("Enter Reading..%X",ntStatus);
    	if(NT_SUCCESS(ntStatus))
		{
//		  //DbgPrint("File Readed..%X %X",ssdtaddr.Index,ssdtaddr.orgValue);
	   	  Address = (ULONG)KeServiceDescriptorTable->ServiceTable+ssdtaddr.Index*4;
	          __asm
	         {
				push    eax
				mov     eax, cr0
          	...
          	// 修改SSDT中服务的地址
  	         *((ULONG*)Address) = (ULONG)ssdtaddr.orgValue;
	    ...
	    byteOffset.QuadPart+=sizeof(SST_ENTRY);
          }
     ZwClose(FileHandle);   
  }   
